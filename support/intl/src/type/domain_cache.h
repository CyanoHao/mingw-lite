...
#include "../stl/basic_string_view.h"
#include "../stl/lower_bound.h"
#include "file.h"
#include "raw_data.h"
...
namespace intl
{
  ...
  private:
    struct cache_entry
    {
      ...
      cache_entry() = default;
      cache_entry(string_view key, string &&value)
          : key(key), value(static_cast<string &&>(value))
      {
      }
  ...
  public:
    ...
    string *find_local(string_view key)
    {
    ...
    string *insert_local(string_view key)
    {
      size_t pos = 0;
      while (pos < cache_size_ && cache_[pos].key < key)
        ++pos;

      if (pos < cache_size_ && cache_[pos].key == key)
      ...
      if (cache_size_ == cache_capacity_) {
        ...
        cache_entry *new_entries =
            (cache_entry *)realloc(cache_, new_cap * sizeof(cache_entry));
        if (!new_entries)
          return nullptr;
        cache_ = new_entries;
      ...
      for (size_t i = cache_size_; i > pos; --i)
        cache_[i] = static_cast<cache_entry &&>(cache_[i - 1]);

      cache_[pos].key = string_view(key.data(), key.size());
      new (&cache_[pos].value) string();
      ++cache_size_;
      return &cache_[pos].value;
    }
  ...
  public:
    char *gettext(string_view msgid)
    {
      ...
      // try find in local cache
      if (string *val = find_local(msgid))
        return val->data();
      ...
      if (string *val = insert_local({t, length})) {
        *val = static_cast<string &&>(abuf);
        return val->data();
      }
      return t;
    }
