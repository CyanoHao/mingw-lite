...
namespace mingw_thunk::internal
{
  ...
  struct basic_string
  {
    ...
    basic_string() noexcept : _s{._is_long = false, ._size = 0, ._data = {0}}
    {
    }
    ...
    basic_string(size_t count, CharT ch) noexcept
    {
      if (count < _sso_size) {
        ...
        _l._is_long = true;
        _l._data = (CharT *)malloc(capacity * sizeof(CharT));
        if (!_l._data) {
          _l._capacity = 0;
          _l._size = npos;
          return;
    ...
    basic_string(const CharT *str, size_t length = npos) noexcept
    {
      ...
      if (length < _sso_size) {
        _set_short(length);
        memcpy(_s._data, str, length * sizeof(CharT));
      } else {
        const size_t capacity = _aligned_capacity(length);
        _l._is_long = true;
        _l._data = (CharT *)malloc(_l._capacity * sizeof(CharT));
        if (!_l._data) {
          _l._capacity = 0;
          _l._size = npos;
          return;
        ...
        _l._capacity = capacity;
        _l._size = length;
        memcpy(_l._data, str, length * sizeof(CharT));
        _l._data[length] = 0;
      }
    }
      ...
      if (count <= size()) {
        ...
        if (_is_long()) {
          if (count > _l._capacity) {
            ...
            CharT *new_data =
                (CharT *)realloc(_l._data, new_capacity * sizeof(CharT));
            if (!new_data) {
              free(_l._data);
              _set_invalid();
              return;
          ...
          if (count < _sso_size) {
            ...
            size_t new_capacity = _aligned_capacity(count);
            CharT *new_data = (CharT *)malloc(new_capacity * sizeof(CharT));
            if (!new_data) {
              _set_invalid();
              return;
            }
            memcpy(new_data, _s._data, _s._size * sizeof(CharT));
            for (size_t i = _s._size; i < count; ++i)
              new_data[i] = ch;
            new_data[count] = 0;
            _l._is_long = true;
