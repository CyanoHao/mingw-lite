...
namespace mingw_thunk::internal
{
  ...
  {
    return _isatty(fd) && is_console((HANDLE)_get_osfhandle(fd));
  }
  ...
  private:
    void flush_range_no_lock(HANDLE console, size_t end)
    {
      ...
      if (end < buffer.size()) {
        libc::memmove(buffer.data(), buffer.data() + end, buffer.size() - end);
  ...
  {
  private:
    // this class is used for file descriptors that are attached to a console.
    // a simple lock is okay, as the bottleneck is always the console.
    bool lock;
    stl::map<int, io_buffer> buffer_map;

  private:
    // we use `-fno-threadsafe-statics` so we cannot use function-scope static.
    static fd_io_buffer instance;

  public:
    static io_buffer &get(int fd)
    {
      instance.acquire();
      io_buffer &buf = instance.buffer_map[fd];
      instance.release();
      return buf;
    }

  private:
    fd_io_buffer() : lock(false)
    {
    }
  ...
  inline fd_io_buffer fd_io_buffer::instance;
