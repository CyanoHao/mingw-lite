...
namespace mingw_thunk
{
  ...
  {
    __DISPATCH_THUNK_2(ReadConsoleInputW,
                       i::is_nt(),
                       &__ms_ReadConsoleInputW,
                       &f::win9x_ReadConsoleInputW);

    return dllimport_ReadConsoleInputW(
  ...
  namespace f
  {
    ...
    BOOL __stdcall win9x_ReadConsoleInputW(_In_ HANDLE hConsoleInput,
                                           _Out_ PINPUT_RECORD lpBuffer,
                                           _In_ DWORD nLength,
                                           _Out_ LPDWORD lpNumberOfEventsRead)
    {
      DWORD read = 0;
      if (!__ms_ReadConsoleInputA(hConsoleInput, lpBuffer, nLength, &read))
        return FALSE;
