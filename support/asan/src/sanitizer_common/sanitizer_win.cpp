...
// MinGW-lite: the #pragma comment(lib,...) directives below were removed;
// these libraries (psapi, synchronization, advapi32) are provided via
// libsanitizer.spec instead, since GNU ld ignores #pragma comment(lib).
...
// For WaitOnAddress (library provided via libsanitizer.spec).
...
// MinGW-lite: GCC section attribute replaces #pragma section + __declspec(allocate).
__attribute__((section(".CRT$XID"))) int (*__run_atexit)() = RunAtexit;
#endif
