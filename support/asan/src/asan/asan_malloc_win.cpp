...
extern "C" {
__attribute__((noinline)) size_t _msize(void *ptr) {
  GET_CURRENT_PC_BP_SP;
  (void)sp;
  return asan_malloc_usable_size(ptr, pc, bp);
}

__attribute__((noinline)) size_t _msize_base(void *ptr) { return _msize(ptr); }
...
__attribute__((noinline)) void _free_dbg(void *ptr, int) { free(ptr); }

__attribute__((noinline)) void _free_base(void *ptr) { free(ptr); }
...
__attribute__((noinline)) void *_malloc_base(size_t size) { return malloc(size); }

__attribute__((noinline)) void *_malloc_dbg(size_t size, int, const char *, int) {
  return malloc(size);
}
...
__attribute__((noinline)) void *_calloc_base(size_t nmemb, size_t size) {
  return calloc(nmemb, size);
}

__attribute__((noinline)) void *_calloc_dbg(size_t nmemb, size_t size, int,
                                       const char *, int) {
  return calloc(nmemb, size);
}

__attribute__((noinline)) void *_calloc_impl(size_t nmemb, size_t size,
                                        int *errno_tmp) {
  return calloc(nmemb, size);
}
...
__attribute__((noinline)) void *_realloc_dbg(void *ptr, size_t size, int) {
  UNREACHABLE("_realloc_dbg should not exist!");
  return 0;
}

__attribute__((noinline)) void *_realloc_base(void *ptr, size_t size) {
  return realloc(ptr, size);
}

__attribute__((noinline)) void *_recalloc(void *p, size_t n, size_t elem_size) {
  if (!p)
...
__attribute__((noinline)) void *_recalloc_base(void *p, size_t n, size_t elem_size) {
  return _recalloc(p, n, elem_size);
}

__attribute__((noinline)) void *_expand(void *memblock, size_t size) {
  // _expand is used in realloc-like functions to resize the buffer if possible.
  // We don't want memory to stand still while resizing buffers, so return 0.
  return 0;
}

__attribute__((noinline)) void *_expand_dbg(void *memblock, size_t size) {
