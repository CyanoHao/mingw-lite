...
def read_sidecar(lib_path: Path) -> Optional[dict]:
...
def _def_line(entry: ExportEntry) -> str:
  ...
  return base if not qualifiers else base + ' ' + ' '.join(qualifiers)

def sidecar_to_dynamic(sidecar: dict, overlay_symbols: Set[str]) -> Dict[str, ImportSymbols]:
  ...
  for dll_name, dll_data in sidecar.get('imports', {}).items():
    ...
    for entry in dll_data.get('exports', []):
...
def check_static_objects(
  ...
  with TemporaryDirectory() as tmpdir:
    ...
    result: Set[str] = set()
    for obj_name in static:
      nm_result = subprocess.run(
        ['llvm-nm', '-g', '--defined-only', obj_name],
        capture_output = True,
        text = True,
        cwd = tmpdir,
      )
...
def read_alias_sidecar_dynamic(sidecar: dict) -> Tuple[Dict[str, ImportSymbols], Set[str]]:
  dynamic: Dict[str, ImportSymbols] = {}
...
def process_single_import_library(
  ...
  base_sidecar = read_sidecar(imp0)
  if base_sidecar:
    base_dynamic = sidecar_to_dynamic(base_sidecar, overlay_symbols)
    for dll_name, symbols in base_dynamic.items():
      if dll_name not in dynamic:
        dynamic[dll_name] = ImportSymbols(set(), set())
      dynamic[dll_name].normal.update(symbols.normal)
      dynamic[dll_name].alias.update(symbols.alias)
    static = set(base_sidecar.get('static_objects', []))
  else:
    static = set()

  if overlay and static:
    static = check_static_objects(imp0, static, overlay_symbols, overlay_force_override, ver)
